...
@interface ObsUpcomingBookingDetailVC ()
...
@property (strong, nonatomic) UIView *signatureView;
@property (strong, nonatomic) UIButton *signatureImageBtn;

@end

@implementation ObsUpcomingBookingDetailVC
...
- (void)viewDidLoad
{
    [super viewDidLoad];
    // Set screen name.
    self.screenName = SCREEN_BOOKING_UPCOMING_DETAIL;
    ...
    NSString *pickupDateIntStr = [self.bookingVehicleDetail objectForKey:COLUMN_PICKUP_DATE];
    NSString *pickupDateStr = [JpDateUtil getDateStrByMilliSecond:[pickupDateIntStr longLongValue]];
    NSString *driverAction = [self.bookingVehicleDetail objectForKey:COLUMN_DRIVER_ACTION];
    if (([today isEqualToString:pickupDateStr] || [tomorrow isEqualToString:pickupDateStr]) &&
        ![DRIVER_ACTION_OK isEqualToString:driverAction]) {
        _okBtn = [UIButton buttonWithType:UIButtonTypeCustom];
        _okBtn.frame = CGRectMake(8, y+3, [JpUiUtil getScreenWidth]-16, 42);
        [_okBtn setBackgroundColor:[UIColor blackColor]];
        [_okBtn setTitle:@"  OK" forState:UIControlStateNormal];
        [_okBtn setImage:[UIImage imageNamed:@"icon_ok_white"] forState:UIControlStateNormal];
        [_okBtn addTarget:self action:@selector(onOkBtn:) forControlEvents:UIControlEventTouchUpInside];
        [_scrollView addSubview:_okBtn];
        y += _okBtn.frame.size.height + 11;
    }
    
    UIImage * squareImg = [UIImage imageNamed:@"bg_btn_square_white"];
    _signatureView = [[UIView alloc] initWithFrame:CGRectMake(8, y, _scrollView.frame.size.width-16, 160)];
    float viewWidth = _signatureView.frame.size.width;
    // Button
    UIButton *signatureBtn = [UIButton buttonWithType:UIButtonTypeCustom];
    signatureBtn.frame = CGRectMake(0,0,_signatureView.frame.size.width,160);
    ...
    [buttonLabel1 setFrame:CGRectMake(0, _signatureView.frame.size.height-30, viewWidth, 20)];
    ...
    [_signatureView addSubview:signatureBtn];
    [_signatureView addSubview:_signatureImageBtn];
    [_signatureView addSubview:buttonLabel1];
    // [signatureView addSubview:labelRedAsterisk];
    
    if (driverAction && [DRIVER_ACTION_OK isEqualToString:driverAction]) {
        [_scrollView addSubview:_signatureView];
        y += _signatureView.frame.size.height + 5;
    }
    ...
    @try {
        ...
        _gmsMapView = [GMSMapView mapWithFrame:CGRectMake(10, y, [JpUiUtil getScreenWidth]-16, [JpUiUtil getScreenHeight]-100) camera:camera];
        _gmsMapView.myLocationEnabled = YES;
    
        [_scrollView addSubview:_gmsMapView];
        y += _gmsMapView.frame.size.height + 5;
        
        GMSMutablePath *path = [GMSMutablePath path];
        [self getGMSMarker:[self.bookingVehicleDetail objectForKey:COLUMN_PICKUP_ADDRESS] title:@"Pick-up Address" imageName:@"icon_location_1" path:path].map = _gmsMapView;
        NSString *destinationImageName = @"icon_location_2";
        if (stop1Address && [stop1Address length]>8) {
            [self getGMSMarker:stop1Address title:@"Stop 1 Address" imageName:@"icon_location_2" path:path].map = _gmsMapView;
            if (stop2Address && [stop2Address length]>8) {
        ...
        if (stop2Address && [stop2Address length]>8) {
            [self getGMSMarker:stop2Address title:@"Stop 2 Address" imageName:@"icon_location_3" path:path].map = _gmsMapView;
        }
        [self getGMSMarker:[self.bookingVehicleDetail objectForKey:COLUMN_DESTINATION] title:@"Destination" imageName:destinationImageName path:path].map = _gmsMapView;
        
        GMSPolyline *polyline = [GMSPolyline polylineWithPath:path];
        polyline.strokeColor = COLOR_BLACK_JP;
        polyline.strokeWidth = 5.f;
        polyline.map = _gmsMapView;
    }
...
- (IBAction)onOkBtn:(id)sender {
    [self showConfirmDialog:@"Are you ok for the job?" tag:OK_SUBMIT];
}
...
-(void)alertView:(UIAlertView *)alertView clickedButtonAtIndex:(NSInteger)buttonIndex
{
    ...
    } else if (alertView.tag == OK_SUBMIT) {
        if (buttonIndex == 1)
        {
            NSString *userId = [JpDataUtil getValueFromUDByKey:KEY_OBS_USER_ID];
            NSString *userName = [JpDataUtil getValueFromUDByKey:KEY_OBS_USER_NAME];
            NSDictionary *parameters = @{KEY_USER_ID: userId, KEY_USER_NAME: userName};
            NSString *urlStr = [NSString stringWithFormat:@"web/01/%@/driverOk", [self.bookingVehicleDetail objectForKey:COLUMN_BOOKING_VEHICLE_ITEM_ID]];
            [[ObsWebAPIClient sharedClient] POST:urlStr parameters:parameters success:^(NSURLSessionDataTask *task, id responseObject) {
                NSMutableDictionary *respondDic = responseObject;
                NSString *result = [respondDic valueForKey:KEY_RESULT];
                if ([result isEqualToString:VALUE_SUCCESS]) {
                    ...
                    alertView.tag = OK_SUBMIT_SUCCESS;
                    [alertView show];
                } else {
    ...
    } else if (alertView.tag == OK_SUBMIT_SUCCESS) {
        CGRect oldFrame = _gmsMapView.frame;
        _gmsMapView.frame = CGRectMake(oldFrame.origin.x, oldFrame.origin.y-_okBtn.frame.size.height+_signatureView.frame.size.height + 5 - 8, oldFrame.size.width, oldFrame.size.height);
        CGRect signatureFrame = _signatureView.frame;
        _signatureView.frame = CGRectMake(signatureFrame.origin.x, signatureFrame.origin.y-_okBtn.frame.size.height - 8, signatureFrame.size.width, signatureFrame.size.height);
        [_scrollView addSubview:_signatureView];
        _okBtn.hidden = YES;
    ...
    if (vehicleInfoStr && [vehicleInfoStr length]>1) {
        @try {
            ...
            if (![oldDriverLoginUserId isEqualToString:driverLoginUserId]) {
                ...
                NSDictionary *parameters = @{KEY_USER_ID:userId, KEY_USER_NAME:userName, @"driverUserId":vehicleId, @"bookingVehicleItemId":bookingVehicleItemId, @"driverUserName":driverUserName, @"deviceName":[JpSystemUtil getDeviceName], @"isDriverAccept":@"no", @"past":@"no",@"driverMobileNumber":driverMobileNumber,@"driverVehicle":driverVehicle,@"driverLoginUserId":driverLoginUserId,@"assignDriverUserId":assignDriverUserId, @"assignDriverUserName":assignDriverUserName, @"assignDriverMobilePhone":assignDriverMobilePhone};
