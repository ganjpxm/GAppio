...
@interface ObsdBookingVehicleDetailVC ()
...
@implementation ObsdBookingVehicleDetailVC
...
- (void)viewDidLoad
{
    ...
    scrollView.frame = CGRectMake(0, 5, 320, [JpUiUtil getWindowHeight]+startHeightOffset);
    ...
    NSString *bookByPerson = [NSString stringWithFormat:@"%@ %@ %@", [self.bookingVehicleDetail objectForKey:COLUMN_BOOKING_USER_GENDER], [self.bookingVehicleDetail objectForKey:COLUMN_BOOKING_USER_NAME],[self.bookingVehicleDetail objectForKey:COLUMN_BOOKING_USER_SURNAME]];
    ...
    NSString *assigner = [self.bookingVehicleDetail objectForKey:COLUMN_AGENT_USER_NAME];
    ...
    if ([remark length]>1 && ![remark isEqualToString:@"<null>"]) {
        UILabel *remarkLabel = [[UILabel alloc]initWithFrame:CGRectMake(5, 10, 290, 30)];
        ...
        UIView *remarkView = [[UIView alloc] initWithFrame:CGRectMake(10, y, 300, remarkLabelHeight+20)];
...
- (UIView *)createBoxViewWithStartY:(long)y firstText:(NSString*)firstText secondText:(NSString*)secondText tag:(NSInteger)tag
{
    ...
    UILabel *firstLabel = [[UILabel alloc]initWithFrame:CGRectMake(5, 10, width, 30)];
    ...
    UILabel *secondLabel = [[UILabel alloc]initWithFrame:CGRectMake(5, firstLabelHeight+10, width, 30)];
    ...
    UIView *view = [[UIView alloc] initWithFrame:CGRectMake(10, y, 300, firstLabelHeight + secondLabelHeight + 20)];
    ...
    if (tag>0) {
        ...
        if (tag==MAP_DIRECTION) {
            [callBtn setImage:[UIImage imageNamed:@"icon_direction_green"] forState:UIControlStateNormal];
            [callBtn addTarget:self action:@selector(onOpenMapBtn:) forControlEvents:UIControlEventTouchUpInside];
        } else if (tag==MAP_ROUTE) {
            [callBtn setImage:[UIImage imageNamed:@"icon_route_green"] forState:UIControlStateNormal];
            [callBtn addTarget:self action:@selector(onOpenMapBtn:) forControlEvents:UIControlEventTouchUpInside];
        } else {
